...
<xsl:output method="xml" encoding="UTF-8" indent="no" />
	...
	<xsl:template match="*:quote">		
		<xsl:copy>
			...
			<xsl:attribute name="style" select="if(ancestor::*:epigraph) then('txt_Epigraphe') else if(@xml:lang) then('txt_Citation_italique') else('txt_Citation')" />
	...
	<xsl:template match="*:table">
		<xsl:copy>
			...
			<xsl:if test="following-sibling::*:head">
				<head xmlns="http://www.tei-c.org/ns/1.0"><xsl:apply-templates select="following-sibling::*:head/node()" /></head>
			</xsl:if>
			<xsl:apply-templates select="node()" />
		</xsl:copy>
	</xsl:template>
